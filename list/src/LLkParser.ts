...
export class LLkParser {
  ...
  private fill(i: number) {
    let j = this.lookaheadIndex(this.lookaheadCount)
    for (; j !== this.lookaheadIndex(i); j = this.lookaheadIndex(j + 1)) {
      this.lookaheadTokens[j] = this.input.nextToken()
    }
    this.lookaheadTokens[j] = this.input.nextToken()
  ...
  consume() {
    this.index = this.lookaheadIndex(this.index + 1)
    this.lookaheadCount--
  }
