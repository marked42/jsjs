...
export class MemoizationParser {
  ...
  private static FAILED = -1
  alreadyParsedRule(memo: Map<number, number>) {
    const value = memo.get(this.index)

    if (value === undefined) {
      return false
    }

    if (value === MemoizationParser.FAILED) {
  ...
  list() {
    ...
    if (this.isSpeculating() && this.alreadyParsedRule(this.listMemo)) {
      return
    }

    let pos = MemoizationParser.FAILED
    try {
      this._list()
      pos = this.index
    } finally {
      this.listMemo.set(start, pos)
    }
  }

  private listMemo = new Map<number, number>()
