...
export class CParser extends ParseletParser {
  ...
  registerSequenceExpression() {
    // TODO: need Operator of specified left right precedence
    const comma = new PrecedentialOperator(Precedence.Comma, {
      hasPrecedingOperand: true,
      hasFollowingOperand: true,
      isFirstOperator: false,
      isLastOperator: false,
    ...
    // 两种构造方法
    // 1. 右结合的二元运算符，对结果进行处理成平行的ast
    // 2. 第一个逗号的precedence，和后面的逗号不同，需要状态记录
